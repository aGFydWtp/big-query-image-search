...
- **CloudRunIam 追補（Revalidation, 2026-06-19・下流 image-search-api Requirement 3.2/3.3/5.4）**: 検索 API の keyless V4 署名 URL 発行のため、Cloud Run SA に次の2ロールを追加した。(1) `roles/storage.objectViewer`（images バケットスコープ、署名対象オブジェクト読取）、(2) `roles/iam.serviceAccountTokenCreator`（**Run SA 自身をリソースとして**バインド、signBlob による自己署名）。あわせて signBlob ランタイムのため `iamcredentials.googleapis.com` を `apis.tf` の有効化リストへ追加。いずれも非 authoritative な `*_iam_member` で最小権限（自己 signBlob ＋ バケットスコープ読取）を維持。下流 image-search-api がこの IAM を消費し署名 URL をフル実装する。
