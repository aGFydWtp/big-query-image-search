...
- Failure filtering: ソース側で「ステータス成功 AND `embedding IS NOT NULL` AND `ARRAY_LENGTH(embedding)=3072`」を満たす行のみを MERGE 対象とし、失敗・空・次元不正行は反映せず再実行で再取込（詳細は Error Handling 参照、実列名は実装前検証スパイクで確定）。
...
- 埋め込み生成の一部失敗／空結果: 成功行のみ MERGE、失敗 URI を再実行（4.4）。失敗判定は以下を MERGE の `WHEN MATCHED/NOT MATCHED ... AND` 条件およびソース側 `WHERE` で明示的に行う:
  - `AI.GENERATE_EMBEDDING` 出力のステータス/エラー列（実列名・型は実装前検証スパイクで確定。慣例的に `status`／`ml_generate_embedding_status` 相当が空文字なら成功）が「成功」を示す行のみを MERGE 対象とする。
