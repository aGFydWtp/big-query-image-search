...
2. リモートモデル（`gemini-embedding-2-preview` 参照、Terraform 管理外の SQL DDL）
...
- 取込と検索が同一モデル（`gemini-embedding-2-preview`）・同一次元・同一距離タイプで連携できる共有契約を確定する。
...
- `sql/remote_model.sql`: リモートモデルの `CREATE OR REPLACE MODEL ... REMOTE WITH CONNECTION OPTIONS(ENDPOINT='gemini-embedding-2-preview')`（Terraform 外）。
...
- `gemini-embedding-2-preview` のエンドポイント名／提供形態の変更 → リモートモデル DDL を再検証。**Preview ステージ**のため GA 化・廃止・名称変更の可能性があり、提供状況を定期確認する。
- **共有契約修正履歴（Task 0, 2026-06-19）**: エンドポイント名を `gemini-embedding-2`→`gemini-embedding-2-preview` に修正、`AI.GENERATE_EMBEDDING` 出力列を `embedding`/`status` に確定。次元 3072・モデルオブジェクト名 `gemini_embedding_model`・距離タイプ COSINE は不変。下流 `image-search-api` へエンドポイント名整合の Revalidation を起票済み（モデルオブジェクト名注入のため機能的契約は不変＝再実装不要、ドキュメント整合のみ）。
...
graph TB
    ...
    Upstream --> RemoteModel[remote model gemini-embedding-2-preview]
...
| 埋め込みモデル | リモートモデル → `gemini-embedding-2-preview` | 画像→ベクトル生成 | マルチモーダル、`output_dimensionality=3072`、**Preview ステージ**（GA 代替は `multimodalembedding@001`／最大 1408 次元） |
...
├── remote_model.sql        # リモートモデル作成（CREATE OR REPLACE MODEL, ENDPOINT=gemini-embedding-2-preview）
...
sequenceDiagram
    ...
    participant Vertex as gemini-embedding-2-preview
...
| 2.1-2.5 | リモートモデル `gemini-embedding-2-preview`（Terraform 外） | RemoteModel | remote_model.sql |
...
| RemoteModel | DWH/モデル | `gemini-embedding-2-preview` 参照モデル | 2.1-2.5 | BigLake 接続 (P0), dataset (P0) | State |
...
| Field | Detail |
|-------|--------|
| Intent | `gemini-embedding-2-preview` を参照する BigQuery リモートモデル |
| Requirements | 2.1, 2.2, 2.3, 2.4, 2.5 |

**Responsibilities & Constraints**
- `CREATE OR REPLACE MODEL ... REMOTE WITH CONNECTION OPTIONS(ENDPOINT='gemini-embedding-2-preview')`（2.1, 2.4）。
...
- Validation: コンソール「Remote endpoint」が `gemini-embedding-2-preview` であることを確認。
...
**Responsibilities & Constraints**
- `AI.GENERATE_EMBEDDING(MODEL <remote_model>, TABLE <object_table>, STRUCT(3072 AS output_dimensionality))` を実行（4.1）。Object Table（画像, `object_metadata='SIMPLE'`）入力では `content` 列は不要で、絞り込みは `WHERE`/`ORDER BY`/`LIMIT` のみ指定可（Task 0 で確定）。
- 出力列は `embedding`（`ARRAY<FLOAT64>`, 次元 3072）と `status`（`STRING`, 成功時は空文字）。`embedding` を embeddings テーブルの `embedding` 列に、Object Table の URI（`uri` 列）を `image_uri` にマッピング（4.2）。
...
- Failure filtering: ソース側で「`status = ''`（成功）AND `embedding IS NOT NULL` AND `ARRAY_LENGTH(embedding)=3072`」を満たす行のみを MERGE 対象とし、失敗・空・次元不正行は反映せず再実行で再取込（詳細は Error Handling 参照。`AI.GENERATE_EMBEDDING` の成功判定列は `status`（空文字＝成功）で Task 0 にて確定）。
...
- 下流 `image-search-api` への共有契約: テーブル名 `image_embeddings`、列 `image_uri` / `embedding`(dim=3072) / `content_type` / `generated_at`、距離タイプ `COSINE`、リモートモデルのオブジェクト名 `gemini_embedding_model`（エンドポイント `gemini-embedding-2-preview`）。下流 `image-search-api` はクエリ埋め込みで同一のモデルオブジェクト名 `gemini_embedding_model` を参照しなければならない。これらの変更は下流再検証 Trigger。
- 上流からの注入契約: `project_id`, `dataset_id`, `connection_id`, `region`, `bucket_uri` をパラメータ化（ハードコード禁止）。
- 上流出力からのパラメータ導出規約（`gcp-infrastructure` の `terraform output` 値が一次ソース。`params.example` に手順を明記する）:
  - `bucket_uri`: 上流出力は `image_bucket_name`（バケット名のみ、URI ではない）。`bucket_uri = gs://${image_bucket_name}` として導出し、Object Table の `uris` には末尾ワイルドカード（例: `${BUCKET_URI}/*`）を付与する。
  - `dataset_id`: 上流出力 `bigquery_dataset_id` は `project_id.dataset_id` 形式。DDL のテーブル/モデル修飾にはこの値をそのまま使用し、`project_id` 単体が必要な場合は `.` 区切り左側を抽出する。
  - `connection_id`: 上流出力 `bigquery_connection_id` は完全修飾 `projects/{project}/locations/{region}/connections/{connection_id}`。`WITH CONNECTION` にはこの完全修飾値を使用し、`region` 単体が必要な場合は `locations/` セグメントから抽出する（独立した region 出力は上流に無い）。
  - 制約: region は dataset・接続・モデル・テーブルで単一値に統一する基準（前掲）。`connection_id` の `region` セグメントと dataset の実ロケーションが一致することを `params.example` 適用時に確認する。
...
- 埋め込み生成の一部失敗／空結果: 成功行のみ MERGE、失敗 URI を再実行（4.4）。失敗判定は以下を MERGE の `WHEN MATCHED/NOT MATCHED ... AND` 条件およびソース側 `WHERE` で明示的に行う:
  - `AI.GENERATE_EMBEDDING` 出力の `status` 列（`STRING`, 空文字＝成功。Task 0 で確定。`ml_generate_embedding_status` は旧 `ML.GENERATE_EMBEDDING` 用であり本仕様では使用しない）が「成功」を示す行（`status = ''`）のみを MERGE 対象とする。
  - 併せて `embedding IS NOT NULL` かつ `ARRAY_LENGTH(embedding) = 3072`（共有契約次元）を満たす行のみ投入し、次元不一致・空ベクトルを排除する。
  - 失敗行は MERGE 対象から除外され embeddings テーブルへ反映されないため、対象 URI レンジの再実行で未処理として再取込される（冪等）。
  - 失敗の可視化: 同一バッチのソース集合に対し「総数 / 成功数 / 失敗数（ステータス非成功または次元不正）」を集計するクエリを `validate.sql` に含め、失敗有無を検証可能にする（4.4）。
...
- RemoteModel: `remote_model.sql` 実行後、モデルが存在し Remote endpoint が `gemini-embedding-2-preview`（2.1, 2.5）。
