...
# 実装フェーズ: Task 6.1 上流 IAM 依存追跡と署名 URL 実機検証ゲート（2026-06-19, `/kiro-impl` 実行）

署名 URL（keyless V4 / IAM `signBlob`）に必須の上流 IAM 追補（design「Upstream Prerequisite」, Requirement 3.2/3.3/5.4）の依存追跡と実機検証結果。

## (1) 上流 IAM 追補の依存追跡 / 起票 — 完了

`gcp-infrastructure` の `terraform/iam.tf` に下記 2 バインドが**既に実装済み**（commit `9e7b233 feat(gcp-infrastructure): 署名URL用 Run SA IAM を追補（image-search-api Rev）`）。トレーサビリティコメントに「下流 image-search-api Requirement 3.2/3.3/5.4」を明記し、Revalidation Trigger として追跡されている。

- `google_service_account_iam_member.run_sa_self_token_creator`: Run SA 自身をリソースとして `roles/iam.serviceAccountTokenCreator`（keyless V4 署名の `signBlob`）。
- `google_storage_bucket_iam_member.run_sa_image_object_viewer`: images バケットスコープの `roles/storage.objectViewer`（署名対象オブジェクト読取、最小権限）。

## (2) 実機適用状態の確認 — 適用済み（LIVE）

実プロジェクト `image-search-6c457e` で `gcloud` 実機確認:

- Run SA `img-search-run@image-search-6c457e.iam.gserviceaccount.com` の IAM ポリシーに `roles/iam.serviceAccountTokenCreator`（メンバー＝当該 SA 自身）が**付与済み**。
- images バケット `image-search-6c457e-imgsearch-images` の IAM ポリシーに Run SA の `roles/storage.objectViewer` が**付与済み**。
- すなわち keyless 署名の IAM 前提は LIVE。**Run SA として稼働するデプロイ済みサービスは署名 URL を発行可能**な権限状態にある。

## (3) 署名 URL 実機発行の確認方法と現状

- **コア検索の成立（部分失敗パス）= 検証済み**: 署名生成失敗時に当該項目は URL を省略し他結果（`image_uri`・`score`）を 200 で返すこと（Requirement 4.5）は、Task 3.3 の `SignBytes` 失敗注入単体テスト（`TestSignedURLs_PartialFailure`）と Task 4.1 のハンドラ部分失敗テスト（`TestHandler_SignerPartialFailureStillReturns200`）で検証済み。署名 URL が未発行でもコア検索価値は毀損しない。
- **end-to-end の実署名発行 = デプロイ時の手動ゲート**: サービスが Run SA として `signBlob` を実行する経路の実署名は、(i) サービスを Run SA でデプロイ、(ii) images バケットに署名対象オブジェクトが存在、の 2 条件下で確認する。本セッションでは外部からの Run SA インパーソネーションによる実署名検証は**本番資格情報の探査として実行を見送り**（自律スコープ外）、かつバケットは現時点で空のため、実署名の end-to-end 確認はデプロイ時に持ち越す。IAM 前提（上記(2)）は LIVE で確認済みのため、ブロッカーは権限ではなくデプロイ/オブジェクト投入の運用手順のみ。
- **DoD 判定**: 「上流起票が記録され（(1)）、追補が適用され（(2) LIVE）、（未署名時も）部分失敗パスでコア検索が 200 を返すことを確認できる（(3) テスト済み）」を満たす。実署名 end-to-end はデプロイ時ゲート（`docs/runbook.md` 検索 API デプロイ節 + 署名 URL 上流 IAM 前提）として残す。
